...
interface StateMutationTransaction {
}
...
class StateMutationTransactionToken {
    internal val previous: StateMutationTransaction?

    constructor(previous: StateMutationTransaction? = null) {
...
class CapturedRead<T> {
    val value: T
    val transaction: StateMutationTransaction?

    constructor(value: T, transaction: StateMutationTransaction?) {
...
class StateTracking {
    ...
    companion object {
        ...
        private val transactionStack: ThreadLocal<MutableList<StateMutationTransaction>> = ThreadLocal()
        ...
        private val readCursor: ThreadLocal<StateMutationTransaction?> = ThreadLocal()
        ...
        fun pushTransaction(transaction: StateMutationTransaction): StateMutationTransactionToken {
            val token = StateMutationTransactionToken(previous = currentTransaction)
        ...
        fun popTransaction(token: StateMutationTransactionToken) {
            val stack_0 = transactionStack.get()
            if (stack_0 == null) {
                return
            }
            if (!stack_0.isEmpty()) {
                stack_0.removeAt(stack_0.size - 1)
            }
            if (stack_0.isEmpty()) {
                transactionStack.set(null)
            }
        }
        ...
        val currentTransaction: StateMutationTransaction?
            get() {
        ...
        fun recordRead(transaction: StateMutationTransaction?) {
            if (transaction == null) {
                return
            }
            if (readCursor.get() == null) {
        ...
        fun <T> captureRead(body: () -> T): CapturedRead<T> {
            val outer = readCursor.get()
            readCursor.set(null)
            try {
                val value = body()
                val inner = readCursor.get()
        ...
        fun clearReadCursor(): Unit = readCursor.set(null)
        ...
        fun captureLastReadAndClear(): StateMutationTransaction? {
            val transaction = readCursor.get()
            if (transaction != null) {
                readCursor.set(null)
            }
            return transaction
