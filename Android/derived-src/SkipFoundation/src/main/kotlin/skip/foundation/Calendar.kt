...
class Calendar: Codable, KotlinConverting<java.util.Calendar>, MutableStruct {
    ...
    fun dateInterval(of: Calendar.Component, start: InOut<Date>, interval: InOut<Double>, for_: Date): Boolean {
        ...
        when (component) {
            ...
            Calendar.Component.hour -> {
                val originalTimeInMillis = platformCal.timeInMillis.sref()
                val minuteMillis = platformCal.get(java.util.Calendar.MINUTE) * 60 * 1000
                val secondMillis = platformCal.get(java.util.Calendar.SECOND) * 1000
                val millisecond = platformCal.get(java.util.Calendar.MILLISECOND)
                platformCal.timeInMillis = originalTimeInMillis - Long(minuteMillis + secondMillis + millisecond)
                start.value = Date(platformValue = platformCal.time)
                interval.value = TimeInterval(60 * 60)
                return true
            }
            Calendar.Component.day, Calendar.Component.dayOfYear, Calendar.Component.weekday, Calendar.Component.weekdayOrdinal -> {
                clearTime(in_ = platformCal)
                start.value = Date(platformValue = platformCal.time)
                val nextDayCal = (platformCal.clone() as java.util.Calendar).sref()
                nextDayCal.add(java.util.Calendar.DAY_OF_MONTH, 1)
                interval.value = TimeInterval((nextDayCal.timeInMillis - platformCal.timeInMillis).toDouble() / 1000.0)
                return true
            }
            Calendar.Component.month -> {
                platformCal.set(java.util.Calendar.DAY_OF_MONTH, 1)
                clearTime(in_ = platformCal)
                start.value = Date(platformValue = platformCal.time)
                val nextMonthCal = (platformCal.clone() as java.util.Calendar).sref()
                nextMonthCal.add(java.util.Calendar.MONTH, 1)
                interval.value = TimeInterval((nextMonthCal.timeInMillis - platformCal.timeInMillis).toDouble() / 1000.0)
                return true
            }
            Calendar.Component.weekOfMonth, Calendar.Component.weekOfYear -> {
                ...
                clearTime(in_ = platformCal)
                start.value = Date(platformValue = platformCal.time)
                val nextWeekCal = (platformCal.clone() as java.util.Calendar).sref()
                nextWeekCal.add(java.util.Calendar.WEEK_OF_YEAR, 1)
                interval.value = TimeInterval((nextWeekCal.timeInMillis - platformCal.timeInMillis).toDouble() / 1000.0)
                return true
            }
            Calendar.Component.quarter -> {
            ...
            Calendar.Component.year -> {
                ...
                clearTime(in_ = platformCal)
                start.value = Date(platformValue = platformCal.time)
                val nextYearCal = (platformCal.clone() as java.util.Calendar).sref()
                nextYearCal.add(java.util.Calendar.YEAR, 1)
                interval.value = TimeInterval((nextYearCal.timeInMillis - platformCal.timeInMillis).toDouble() / 1000.0)
                return true
            }
            Calendar.Component.era -> {
        ...
        do {
            ...
            try {
                ...
                if (matchtarget_0 != null) {
                    ...
                    if (components.weekOfYear != null) {
                        if (direction == Calendar.SearchDirection.forward) {
                            searchingDate = result.newSearchDate.sref()
                        } else {
                            searchingDate = (this.date(byAdding = Calendar.Component.weekOfYear, value = -1, to = matchDate) ?: matchDate.addingTimeInterval(-7.0 * 24.0 * 60.0 * 60.0)).sref()
                        }
                    } else {
    ...
    private fun dateAfterMatchingMonth(startingAt: Date, components: DateComponents, direction: Calendar.SearchDirection, strictMatching: Boolean): Date? {
        ...
        if (month_0 != dateMonth) {
            do {
                ...
                val searchDate: Date
                if (direction == Calendar.SearchDirection.backward) {
                    val lateLastMonth = foundRange_3.start.addingTimeInterval(-1)
                    searchDate = (this.dateInterval(of = Calendar.Component.month, for_ = lateLastMonth)?.start ?: foundRange_3.start.addingTimeInterval(-86400)).sref()
                } else {
                    searchDate = foundRange_3.start.addingTimeInterval(foundRange_3.duration)
                }
    ...
    private fun dateAfterMatchingWeekOfYear(startingAt: Date, components: DateComponents, direction: Calendar.SearchDirection): Date? {
        ...
        do {
            ...
            if (direction == Calendar.SearchDirection.backward) {
                val lateLastWeek = foundRange_4.start.addingTimeInterval(-1)
                val searchDate = (this.dateInterval(of = Calendar.Component.weekOfYear, for_ = lateLastWeek)?.start ?: foundRange_4.start.addingTimeInterval(-foundRange_4.duration)).sref()
        ...
        val shouldPreserveSearchTime = components.year == null && components.weekday == null && components.weekdayOrdinal == null && components.day == null && components.dayOfYear == null && components.hour == null && components.minute == null && components.second == null && components.nanosecond == null
        if (shouldPreserveSearchTime) {
            var adjustedComponents = this.dateComponents(setOf(Calendar.Component.yearForWeekOfYear, Calendar.Component.weekOfYear, Calendar.Component.weekday), from = result)
            val time = this.dateComponents(setOf(Calendar.Component.hour, Calendar.Component.minute, Calendar.Component.second), from = startingAt)
            ...
            this.date(from = adjustedComponents)?.let { adjusted ->
    ...
    private fun dateAfterMatchingWeekOfMonth(startingAt: Date, components: DateComponents, direction: Calendar.SearchDirection): Date? {
        ...
        if (weekOfMonth_0 == dateWeekOfMonth) {
            val shouldReturnWeekStart = components.year == null && components.month == null && components.weekday == null && components.weekdayOrdinal == null && components.day == null && components.hour == null
            if (shouldReturnWeekStart) {
                this.dateInterval(of = Calendar.Component.weekOfMonth, for_ = startingAt)?.let { weekInterval ->
                    return weekInterval.start.sref()
                }
            }
        ...
        // After this point, result is at least startDate.
        var result = startingAt.sref()
        if (components.month != null && direction == Calendar.SearchDirection.backward) {
        ...
        if ((components.month != null) && (components.weekday == null) && (direction == Calendar.SearchDirection.backward) && (this.component(Calendar.Component.weekday, from = result) == 1)) {
            this.date(byAdding = Calendar.Component.day, value = 1, to = result)?.let { adjusted ->
                result = adjusted.sref()
            }
        }
    ...
    private fun dateAfterMatchingDayOfYear(startingAt: Date, components: DateComponents, direction: Calendar.SearchDirection): Date? {
        ...
        do {
            ...
            if (direction == Calendar.SearchDirection.backward) {
                val lateYesterday = foundRange_6.start.addingTimeInterval(-1)
                val searchDate = (this.dateInterval(of = Calendar.Component.dayOfYear, for_ = lateYesterday)?.start ?: foundRange_6.start.addingTimeInterval(-foundRange_6.duration)).sref()
    ...
    private fun _adjustedDate(unadjustedMatchDate: Date, startingAfter: Date, allowStartDate: Boolean = false, matching: DateComponents, adjustedMatchingComponents: DateComponents, matchingPolicy: Calendar.MatchingPolicy, repeatedTimePolicy: Calendar.RepeatedTimePolicy, direction: Calendar.SearchDirection, inSearchingDate: Date, previouslyReturnedMatchDate: Date?): Calendar.SearchStepResult {
        ...
        var matchDate = baseMatchDate_0.sref()
        if ((repeatedTimePolicy == Calendar.RepeatedTimePolicy.first) && (matchingComponents.hour != null)) {
            val hasExplicitDateComponent = matchingComponents.era != null || matchingComponents.year != null || matchingComponents.yearForWeekOfYear != null || matchingComponents.dayOfYear != null || matchingComponents.quarter != null || matchingComponents.month != null || matchingComponents.day != null || matchingComponents.weekday != null || matchingComponents.weekdayOrdinal != null || matchingComponents.weekOfMonth != null || matchingComponents.weekOfYear != null

            if (direction == Calendar.SearchDirection.forward || hasExplicitDateComponent) {
                val earlier = matchDate.addingTimeInterval(-3600.0)
                val isAllowedByDirection: Boolean
                if (direction == Calendar.SearchDirection.forward) {
                    isAllowedByDirection = earlier > start
                } else if (previouslyReturnedMatchDate != null) {
                    isAllowedByDirection = earlier < previouslyReturnedMatchDate
                } else {
                    isAllowedByDirection = earlier < start
                }

                if (isAllowedByDirection && this.date(earlier, containsMatchingComponents = matchingComponents).element1) {
                    matchDate = earlier.sref()
                }
            }
        }

        // Check the components to see if they match what was desired.
        val matchResult = this.date(matchDate, containsMatchingComponents = matchingComponents)
        ...
        this.bumpedDateUpToNextHigherUnitInComponents(searchingDate, matchingComponents, direction, matchDate)?.let { newSearchingDate ->
            searchingDate = newSearchingDate.sref()
        }
        ...
        if (((direction == Calendar.SearchDirection.backward && order == ComparisonResult.orderedAscending) || (direction == Calendar.SearchDirection.forward && order == ComparisonResult.orderedDescending)) && mismatchedUnits.contains(Calendar.Component.nanosecond) == false) {
            ...
            if (compsToMatch.highestSetUnit == Calendar.Component.hour) {
                val matchHour = this.component(Calendar.Component.hour, from = matchDate)
                val hourAdjustment = if (direction == Calendar.SearchDirection.backward) -3600.0 else 3600.0
                val potentialNextMatchDate = matchDate.addingTimeInterval(hourAdjustment)
        ...
        return SearchStepResult(result = Tuple2(matchDate.sref(), exactMatch), newSearchDate = searchingDate)
    }
    ...
    private fun _adjustedComponents(comps: DateComponents, date: Date, direction: Calendar.SearchDirection): DateComponents {
        ...
        when (highestSetUnit) {
            ...
            Calendar.Component.dayOfYear -> {
                var adjusted = comps.sref()
                val dateDayOfYear = this.component(Calendar.Component.dayOfYear, from = date)
                val dateYear = this.component(Calendar.Component.year, from = date)
                if (direction == Calendar.SearchDirection.backward) {
                    adjusted.year = if ((comps.dayOfYear ?: Int.max) >= dateDayOfYear) dateYear - 1 else dateYear
                } else {
                    adjusted.year = if ((comps.dayOfYear ?: Int.max) > dateDayOfYear) dateYear else dateYear + 1
                }
            ...
            Calendar.Component.weekOfYear -> {
                var adjusted = comps.sref()
                val dateWeekOfYear = this.component(Calendar.Component.weekOfYear, from = date)
                val dateYearForWeekOfYear = this.component(Calendar.Component.yearForWeekOfYear, from = date)
                if (direction == Calendar.SearchDirection.backward) {
                    adjusted.yearForWeekOfYear = if ((comps.weekOfYear ?: Int.max) >= dateWeekOfYear) dateYearForWeekOfYear - 1 else dateYearForWeekOfYear
                } else {
                    adjusted.yearForWeekOfYear = if ((comps.weekOfYear ?: Int.max) > dateWeekOfYear) dateYearForWeekOfYear else dateYearForWeekOfYear + 1
                }
            ...
            Calendar.Component.day -> {
                ...
                if (direction == Calendar.SearchDirection.backward) {
                    ...
                    if (comps.day ?: Int.max >= dateDay) {
                        ...
                        val dateYear = this.component(Calendar.Component.year, from = date)
                        adjusted.month = if (dateMonth == 1) 12 else dateMonth - 1
                        adjusted.year = if (dateMonth == 1) dateYear - 1 else dateYear
                    } else {
