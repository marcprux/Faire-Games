...
open class JSONDecoder: TopLevelDecoder<Data> {
    ...
    open fun decode(type: KClass<Boolean>, from: Data): Boolean {
        val data = from
        return decoder(from = data).singleValueContainer().decode(type = type)
    }

    open fun decode(type: KClass<String>, from: Data): String {
        val data = from
        return decoder(from = data).singleValueContainer().decode(type = type)
    }

    open fun decode(type: KClass<Double>, from: Data): Double {
        val data = from
        return decoder(from = data).singleValueContainer().decode(type = type)
    }

    open fun decode(type: KClass<Float>, from: Data): Float {
        val data = from
        return decoder(from = data).singleValueContainer().decode(type = type)
    }

    open fun decode(type: KClass<Byte>, from: Data): Byte {
        val data = from
        return decoder(from = data).singleValueContainer().decode(type = type)
    }

    open fun decode(type: KClass<Short>, from: Data): Short {
        val data = from
        return decoder(from = data).singleValueContainer().decode(type = type)
    }

    open fun decode(type: KClass<Int>, from: Data): Int {
        val data = from
        return decoder(from = data).singleValueContainer().decode(type = type)
    }

    open fun decode(type: KClass<Long>, from: Data): Long {
        val data = from
        return decoder(from = data).singleValueContainer().decode(type = type)
    }

    open fun decode(type: KClass<UByte>, from: Data): UByte {
        val data = from
        return decoder(from = data).singleValueContainer().decode(type = type)
    }

    open fun decode(type: KClass<UShort>, from: Data): UShort {
        val data = from
        return decoder(from = data).singleValueContainer().decode(type = type)
    }

    open fun decode(type: KClass<UInt>, from: Data): UInt {
        val data = from
        return decoder(from = data).singleValueContainer().decode(type = type)
    }

    open fun decode(type: KClass<ULong>, from: Data): ULong {
        val data = from
        return decoder(from = data).singleValueContainer().decode(type = type)
    }
...
private class JSONDecoderImpl: Decoder {
    ...
    internal fun <T> unwrap(as_: KClass<T>): T where T: Any {
        ...
        if (type == java.math.BigDecimal::class) {
            return (this.unwrapDecimal() as T).sref()
        }
...
if (decodableCompanion != null) {
    ...
    private fun unwrapDecimal(): java.math.BigDecimal {
        val container = JSONSingleValueDecodingContainer(impl = this, codingPath = this.codingPath, json = this.json)
        try {
            ...
            return java.math.BigDecimal(double)
        }
    }
