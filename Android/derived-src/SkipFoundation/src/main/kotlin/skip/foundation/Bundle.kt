...
open class Bundle: SwiftCustomBridged {
    ...
    constructor(location: LocalizedStringResource.BundleDescription) {
        ...
        when (location) {
            ...
            is LocalizedStringResource.BundleDescription.MainCase -> {
                val identifer = Companion.resolveMainBundlePackage()
    ...
    companion object: CompanionClass() {
        ...
        private fun resolveMainBundlePackage(): String {
            val derivedPackage = packageName(forClassName = applicationInfo.className)
            if (hasMainResources(forPackage = derivedPackage)) {
                return derivedPackage
            }
            mainBundlePackageFromLauncher()?.let { activityPackage ->
                if (hasMainResources(forPackage = activityPackage)) {
        ...
        private fun hasMainResources(forPackage: String): Boolean {
            val packageName = forPackage
            val assets = ProcessInfo.processInfo.androidContext.resources.assets.sref()
            val path = packageName.replace(".", "/") + "/Resources"
            assets.list(path)?.let { contents ->
                if (contents.count() > 0) {
                    return true
                }
            }
        ...
        private fun mainBundlePackageFromLauncher(): String? {
            val intent_0 = packageManager.getLaunchIntentForPackage(androidContext.getPackageName())
            if (intent_0 == null) {
                return null
            }
            val className_0 = intent_0.getComponent()?.getClassName()
            if (className_0 == null) {
                return null
            }
            return packageName(forClassName = className_0)
        }
