...
open class NSError: Exception, Error, CustomDebugStringConvertible {
    ...
    open val description: String
        get() {
            val message: String
            val matchtarget_3 = userInfo[NSLocalizedDescriptionKey] as? String
            if (matchtarget_3 != null) {
                val localizedDescription = matchtarget_3
                message = localizedDescription
            } else {
                message = localizedFailureReason ?: "(null)"
            }
            val base = "Error Domain=${domain} Code=${code} \"${message}\""
            if (userInfo.isEmpty) {
                return base
            } else {
...
class CocoaError: Exception, Error {
    val _nsError: NSError

    constructor(_nsError: NSError): super() {
        val error = _nsError
...
class URLError: Exception, Error {
    val _nsError: NSError
    ...
    constructor(code: URLError.Code, userInfo: Dictionary<String, Any> = dictionaryOf()): this(_nsError = NSError(domain = NSURLErrorDomain, code = code.rawValue, userInfo = userInfo)) {
    }
    ...
    enum class Code(override val rawValue: Int, @Suppress("UNUSED_PARAMETER") unusedp: Nothing? = null): RawRepresentable<Int> {
        ...
        @androidx.annotation.Keep
        companion object {
            fun init(rawValue: Int): URLError.Code? {
    ...
    companion object {
        ...
        fun Code(rawValue: Int): URLError.Code? = Code.init(rawValue = rawValue)
    }
}

typealias POSIXErrorCode = POSIXError.Code

class POSIXError: Exception, Error {
    val _nsError: NSError
    ...
    enum class Code(override val rawValue: Int, @Suppress("UNUSED_PARAMETER") unusedp: Nothing? = null): RawRepresentable<Int> {
        ...
        @androidx.annotation.Keep
        companion object {
            fun init(rawValue: Int): POSIXError.Code? {
                return when (rawValue) {
                    1 -> Code.EPERM
                    2 -> Code.ENOENT
                    3 -> Code.ESRCH
    ...
    companion object {
        ...
        fun Code(rawValue: Int): POSIXError.Code? = Code.init(rawValue = rawValue)
    }
}
