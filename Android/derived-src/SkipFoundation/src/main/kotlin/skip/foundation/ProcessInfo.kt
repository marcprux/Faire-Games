...
open class ProcessInfo {
    ...
    class ActivityOptions: OptionSet<ProcessInfo.ActivityOptions, Int>, RawRepresentable<Int>, MutableStruct {
        ...
        override fun assignoptionset(target: ProcessInfo.ActivityOptions) {
            willmutate()
            try {
                assignfrom(target)
            } finally {
        ...
        private constructor(copy: MutableStruct) {
            @Suppress("NAME_SHADOWING", "UNCHECKED_CAST") val copy = copy as ProcessInfo.ActivityOptions
            this.rawValue = copy.rawValue
        }

        override var supdate: ((Any) -> Unit)? = null
        override var smutatingcount = 0
        override fun scopy(): MutableStruct = ProcessInfo.ActivityOptions(this as MutableStruct)
    ...
    enum class ThermalState(override val rawValue: Int, @Suppress("UNUSED_PARAMETER") unusedp: Nothing? = null): RawRepresentable<Int> {
        ...
        @androidx.annotation.Keep
        companion object {
            fun init(rawValue: Int): ProcessInfo.ThermalState? {
    ...
    companion object: CompanionClass() {
        ...
        override fun launch(context: android.content.Context) {
            ...
            context.getPackageManager()?.let { packageManager ->
                packageManager.getPackageInfo(context.getPackageName(), android.content.pm.PackageManager.GET_META_DATA)?.let { packageInfo ->
                    packageInfo.applicationInfo?.metaData.sref()?.let { packageMetaData ->
                        packageMetaData.getString("SKIP_BRIDGE_MODULES")?.let { bridgeModules ->
                            android.util.Log.i("SkipFoundation", "loading SKIP_BRIDGE_MODULES: ${bridgeModules}")
                            val bridgeClass = Class.forName("skip.android.bridge.AndroidBridge").kotlin.sref()
                            android.util.Log.i("SkipFoundation", "calling bridgeClass: ${bridgeClass}")
                            val matchtarget_0 = bridgeClass.companionObject
                            if (matchtarget_0 != null) {
                                val companionObject = matchtarget_0
                                val matchtarget_1 = companionObject.functions?.find({ it -> it.name == "initBridge" })
                                if (matchtarget_1 != null) {
                                    val initBridge = matchtarget_1
        ...
        override fun ThermalState(rawValue: Int): ProcessInfo.ThermalState? = ThermalState.init(rawValue = rawValue)
    }
