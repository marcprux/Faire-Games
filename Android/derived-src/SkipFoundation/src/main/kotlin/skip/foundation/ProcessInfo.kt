                ...
                packageManager.getPackageInfo(context.getPackageName(), android.content.pm.PackageManager.GET_META_DATA)?.let { packageInfo ->
                    packageInfo.applicationInfo?.metaData.sref()?.let { packageMetaData ->
                        packageMetaData.getString("SKIP_BRIDGE_MODULES")?.let { bridgeModules ->
                            ...
                            val bridgeClass = Class.forName("skip.android.bridge.AndroidBridge")
                            android.util.Log.i("SkipFoundation", "calling bridgeClass: ${bridgeClass}")
                            val companion = bridgeClass.getField("Companion").get(null)
                            val matchtarget_0 = companion?.javaClass?.getMethods()?.find({ it -> it.getName() == "initBridge" })
                            if (matchtarget_0 != null) {
                                val initBridge = matchtarget_0
