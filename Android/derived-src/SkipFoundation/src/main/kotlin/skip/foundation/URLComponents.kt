    ...
    companion object {
        ...
        internal fun queryString(from: Array<URLQueryItem>?): String? {
            ...
            for (item in items.sref()) {
                val name = item.name.addingPercentEncoding(withAllowedCharacters = CharacterSet.urlQueryAllowed) ?: item.name
                val rawValue = item.value ?: ""
                val value = rawValue.addingPercentEncoding(withAllowedCharacters = CharacterSet.urlQueryAllowed) ?: rawValue
                if (!query.isEmpty) {
                    query += "&"
                }
                query += name + "=" + value
