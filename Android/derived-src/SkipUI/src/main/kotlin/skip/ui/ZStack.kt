...
class ZStack: View, Renderable, skip.lib.SwiftProjecting {
            ...
            ComposeContainer(eraseAxis = true, modifier = context.modifier) { modifier ->
                Box(modifier = modifier, contentAlignment = alignment.asComposeAlignment()) { ->
                    ...
                    for (index in 0..<renderables.size) {
                        val renderable = renderables[index].sref()
                        val key = childKey(for_ = renderable, index = index, occurrences = occurrences)
                        androidx.compose.runtime.key(key) { -> renderable.Render(context = contentContext) }
                    }
                }
            }
        } else {
        ...
        AnimatedContent(modifier = modifier, targetState = arguments.renderables, transitionSpec = { ->
            ...
            Box(contentAlignment = alignment.asComposeAlignment()) { ->
                ...
                for (index in 0..<state.size) {
                    val renderable = state[index].sref()
                    ...
                    val key = childKey(for_ = renderable, index = index, occurrences = occurrences)
                    androidx.compose.runtime.key(key) { -> renderable.Render(context = context.content(modifier = modifier)) }
                }
            }
        }, label = "ZStack")
    }
    ...
    private fun childKey(for_: Renderable, index: Int, occurrences: MutableMap<Any, Int>): Any {
        val renderable = for_
        val identity: Any = (TagModifier.on(content = renderable, role = ModifierRole.id)?.value ?: TagModifier.on(content = renderable, role = ModifierRole.tag)?.value ?: index).sref()
