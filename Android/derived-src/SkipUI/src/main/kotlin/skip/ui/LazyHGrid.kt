                    ...
                    LazyHorizontalGrid(state = gridState, modifier = modifier, rows = gridCells, horizontalArrangement = horizontalArrangement, verticalArrangement = verticalArrangement, contentPadding = contentPadding, userScrollEnabled = isScrollEnabled, flingBehavior = flingBehavior) { ->
                        itemCollector.value.initialize(startItemIndex = 0, item = { renderable, _ ->
                            ...
                            items(count = objects.count, key = key) { index ->
                                val scopedContext = context.content(scope = this)
                                Box(contentAlignment = boxAlignment) { -> factory(objects[index], scopedContext).Render(context = scopedContext) }
                            }
                            ...
                            items(count = objectsBinding.wrappedValue.count, key = key) { index ->
                                val scopedContext = context.content(scope = this)
                                Box(contentAlignment = boxAlignment) { -> factory(objectsBinding, index, scopedContext).Render(context = scopedContext) }
                            }
