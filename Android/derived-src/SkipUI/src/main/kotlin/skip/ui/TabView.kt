...
class TabView: View, Renderable, skip.lib.SwiftProjecting {
    ...
    private fun RenderTabViewContent(context: ComposeContext) {
        ...
        val tabs: kotlin.collections.List<Tab?> = tabRenderables.map l@{ it ->
            ...
            val badgeModifier = BadgeModifier.combined(for_ = renderable)
            val matchtarget_1 = renderable.strip() as? Tab
            if (matchtarget_1 != null) {
                var tab = matchtarget_1
                badgeModifier.badge?.let { badge ->
                    tab.badge = badge
                }
                return@l tab
            } else {
            ...
            ComposeContainer(modifier = context.modifier, fillWidth = true, fillHeight = true) { modifier ->
                ...
                Box(modifier = modifier.background(Color.background.colorImpl()).fillMaxSize()) { ->
                    ...
                    NavigationSuiteScaffoldLayout(navigationSuite = l@{ ->
                                ...
                                if (layoutType == NavigationSuiteType.NavigationBar) {
                                    NavigationBar(modifier = options.modifier.semantics { -> testTagsAsResourceId = true }.testTag("skip_ui_automation_tab_bar"), containerColor = options.containerColor, contentColor = options.contentColor, tonalElevation = options.tonalElevation) { ->
                                        for (tabIndex in 0..<tabRenderables.size) {
                                            ...
                                            if (tabs[tabIndex] == null || tabs[tabIndex]?.isHidden == true) {
                                                continue
                                            }
                                            val route = String(describing = tabIndex)
                        ...
                        val defaults = NavDisplayTransitionOptions.tabViewDefaults
                        val transitions = EnvironmentValues.shared._tabViewTransitions?.invoke(defaults) ?: defaults
                        NavDisplay(entries = activeEntries, modifier = Modifier.fillMaxSize(), onBack = { ->
                            if (activeStack.size > 1) {
                                activeStack.removeLastOrNull()
                            }
                        }, transitionSpec = { -> transitions.transitionSpec ?: defaultTransitionSpec<NavKey>()() }, popTransitionSpec = { -> transitions.popTransitionSpec ?: defaultPopTransitionSpec<NavKey>()() }, predictivePopTransitionSpec = { edge -> transitions.predictivePopTransitionSpec ?: defaultPredictivePopTransitionSpec<NavKey>()(edge) })
                    })
                }
            }
        }
...
interface TabContent: View {
    ...
    override fun badge(count: Int): TabContent {
        var tabContent = this.sref()
        tabContent.badge = if (count > 0) Text(String(count)) else null
        return tabContent.sref()
    }

    override fun badge(label: Text?): TabContent {
        var tabContent = this.sref()
        tabContent.badge = label
        return tabContent.sref()
    }

    override fun badge(key: LocalizedStringKey): TabContent {
        var tabContent = this.sref()
        tabContent.badge = Text(key)
        return tabContent.sref()
    }

    override fun badge(resource: LocalizedStringResource): TabContent {
        var tabContent = this.sref()
        tabContent.badge = Text(resource)
        return tabContent.sref()
    }

    override fun badge(label: String): TabContent {
        var tabContent = this.sref()
...
class Tab: TabContent, Renderable, MutableStruct, skip.lib.SwiftProjecting {
    ...
    internal fun RenderImage(context: ComposeContext) {
        ...
        val renderIcon: @Composable () -> Unit = { ->
        ...
        Box(modifier = outerModifier, contentAlignment = androidx.compose.ui.Alignment.Center) { ->
            val matchtarget_14 = badge
            if (matchtarget_14 != null) {
                val badge = matchtarget_14
                BadgedBox(badge = { ->
                    Badge(modifier = Modifier.offset(x = 8.dp, y = -4.dp)) { -> badge.foregroundStyle(Color.white).Render(context = context) }
                }) { -> renderIcon() }
            } else {
                renderIcon()
            }
