...
class TextField: View, Renderable, skip.lib.SwiftProjecting {
    ...
    internal val axis: Axis

    constructor(text: Binding<String>, selection: Binding<TextSelection?>? = null, prompt: Text? = null, isSecure: Boolean = false, axis: Axis = Axis.horizontal, label: () -> View) {
        this.text = text.sref()
    ...
    constructor(getText: () -> String, setText: (String) -> Unit, getSelection: () -> TextSelection?, setSelection: (TextSelection?) -> Unit, prompt: Text?, isSecure: Boolean, bridgedAxis: Int, bridgedLabel: View) {
        ...
        this.prompt = prompt
        this.isSecure = isSecure
        this.axis = Axis(rawValue = bridgedAxis) ?: Axis.horizontal
    }

    constructor(title: String, text: Binding<String>, selection: Binding<TextSelection?>? = null, prompt: Text? = null, axis: Axis = Axis.horizontal): this(text = text, selection = selection, prompt = prompt, axis = axis, label = { ->
    ...
    constructor(titleKey: LocalizedStringKey, text: Binding<String>, selection: Binding<TextSelection?>? = null, prompt: Text? = null, axis: Axis = Axis.horizontal): this(text = text, selection = selection, prompt = prompt, axis = axis, label = { ->
        ComposeBuilder { composectx: ComposeContext ->
    ...
    constructor(titleResource: LocalizedStringResource, text: Binding<String>, selection: Binding<TextSelection?>? = null, prompt: Text? = null, axis: Axis = Axis.horizontal): this(text = text, selection = selection, prompt = prompt, axis = axis, label = { ->
        ComposeBuilder { composectx: ComposeContext ->
    ...
    override fun Render(context: ComposeContext) {
        ...
        val isVertical = axis == Axis.vertical
        var baseModifier = context.modifier.fillWidth()
        if (!isVertical) {
            baseModifier = baseModifier.onPreviewKeyEvent l@{ keyEvent ->
        ...
        val lineLimit = EnvironmentValues.shared.lineLimit
        val maxLines = if (isVertical) (lineLimit ?: Int.max) else 1
        val minLines = if (isVertical && EnvironmentValues.shared._lineLimitReservesSpace == true) (lineLimit ?: 1) else 1
        ...
        }, placeholder = { -> Companion.Placeholder(prompt = prompt ?: label, context = contentContext) }, modifier = baseModifier, textStyle = alignedTextStyle, enabled = EnvironmentValues.shared.isEnabled, singleLine = !isVertical, visualTransformation = visualTransformation, keyboardOptions = keyboardOptions, keyboardActions = keyboardActions, maxLines = maxLines, minLines = minLines, shape = OutlinedTextFieldDefaults.shape, colors = colors)
        EnvironmentValues.shared._material3TextField?.let { updateOptions ->
            options = updateOptions(options)
        }
        val matchtarget_0 = EnvironmentValues.shared._textFieldContentPadding
        if (matchtarget_0 != null) {
            val tfContentPadding = matchtarget_0
            val interactionSource = (options.interactionSource ?: remember { -> MutableInteractionSource() }).sref()
            val cursorColor = (EnvironmentValues.shared._tint ?: Color.primary).colorImpl()
            ...
            val entryColor = Companion.textColor(styleInfo = styleInfo, enabled = options.enabled)
            val entryTextStyle = (if (entryColor == androidx.compose.ui.graphics.Color.Unspecified) options.textStyle else options.textStyle.copy(color = entryColor)).sref()
            BasicTextField(value = options.value, onValueChange = options.onValueChange, modifier = options.modifier, enabled = options.enabled, readOnly = options.readOnly, textStyle = entryTextStyle, keyboardOptions = options.keyboardOptions, keyboardActions = options.keyboardActions, singleLine = options.singleLine, maxLines = options.maxLines, minLines = options.minLines, visualTransformation = options.visualTransformation, interactionSource = interactionSource, cursorBrush = SolidColor(cursorColor), decorationBox = { innerTextField ->
                OutlinedTextFieldDefaults.DecorationBox(value = options.value.text, visualTransformation = options.visualTransformation, innerTextField = innerTextField, placeholder = options.placeholder, label = options.label, leadingIcon = options.leadingIcon, trailingIcon = options.trailingIcon, prefix = options.prefix, suffix = options.suffix, supportingText = options.supportingText, singleLine = options.singleLine, enabled = options.enabled, isError = options.isError, interactionSource = interactionSource, colors = options.colors, contentPadding = tfContentPadding.asPaddingValues(), container = { -> OutlinedTextFieldDefaults.Container(enabled = options.enabled, isError = options.isError, interactionSource = interactionSource, colors = options.colors, shape = options.shape) })
            })
        } else {
