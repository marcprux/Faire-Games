...
interface ModifierProtocol {
    val role: ModifierRole
    ...
    @Composable
    fun Render(content: Renderable, context: ComposeContext)

    @Composable
    fun shouldRenderListItem(content: Renderable, context: ComposeContext): Tuple2<Boolean, (() -> Unit)?> = content.shouldRenderListItem(context = context)

    @Composable
    fun RenderListItem(content: Renderable, context: ComposeContext, modifiers: kotlin.collections.List<ModifierProtocol>): Unit = content.RenderListItem(context = context, modifiers = modifiers.plus(this))
}

/// A modifier that applies a side effect.
@Suppress("MUST_BE_INITIALIZED_OR_FINAL_OR_ABSTRACT")
internal open class SideEffectModifier: ModifierProtocol {
    override val role: ModifierRole
    internal open var action: (@Composable (ComposeContext) -> ComposeResult)? = null

    internal constructor(role: ModifierRole = ModifierRole.unspecified, action: (@Composable (ComposeContext) -> ComposeResult)? = null) {
        this.role = role
        this.action = action
    }
    ...
    override fun Evaluate(content: View, context: ComposeContext, options: Int): kotlin.collections.List<Renderable>? {
        ...
        if (renderables.size == 0) {
            ...
            val actionRenderable = ModifiedContent(content = renderables[0] as Renderable, modifier = this)
            return (listOf(actionRenderable) + renderables.drop(1)).sref()
        }
    }
...
@Suppress("MUST_BE_INITIALIZED_OR_FINAL_OR_ABSTRACT")
internal open class RenderModifier: ModifierProtocol {
    override val role: ModifierRole
    internal open var action: (@Composable (Renderable, ComposeContext) -> Unit)? = null
    ...
    internal open var modifierAction: (@Composable (ComposeContext) -> Modifier)? = null

    internal constructor(role: ModifierRole = ModifierRole.unspecified, action: (@Composable (Renderable, ComposeContext) -> Unit)? = null) {
        this.role = role
        this.action = action
    }

    internal constructor(role: ModifierRole = ModifierRole.unspecified, action: @Composable (ComposeContext) -> Modifier) {
        this.role = role
        this.modifierAction = action
        this.action = { content, context ->
    ...
    @Composable
    override fun Render(content: Renderable, context: ComposeContext) {
        val matchtarget_0 = action
        if (matchtarget_0 != null) {
            val action = matchtarget_0
...
@Suppress("MUST_BE_INITIALIZED_OR_FINAL_OR_ABSTRACT")
internal open class EnvironmentModifier: ModifierProtocol {
    override val role: ModifierRole
    internal val affectsEvaluate: Boolean
    internal open var action: (@Composable (EnvironmentValues) -> ComposeResult)? = null

    internal constructor(role: ModifierRole = ModifierRole.unspecified, affectsEvaluate: Boolean = true, action: (@Composable (EnvironmentValues) -> ComposeResult)? = null) {
        this.role = role
        this.affectsEvaluate = affectsEvaluate
        this.action = action
    }
    ...
    override fun Evaluate(content: View, context: ComposeContext, options: Int): kotlin.collections.List<Renderable>? {
        ...
        if (affectsEvaluate) {
            return EnvironmentValues.shared.setValuesWithReturn(action_0, in_ = l@{ -> return@l EvaluateInternal(content = content, context = context, options = options) })
        } else {
    ...
    @Composable
    private fun EvaluateInternal(content: View, context: ComposeContext, options: Int): kotlin.collections.List<Renderable>? {
    ...
    override fun Render(content: Renderable, context: ComposeContext) {
        ...
        EnvironmentValues.shared.setValues(action_1, in_ = { -> content.Render(context = context) })
    }
}
...
internal class ModifiedContent: View, Renderable, LazyItemFactory {
    internal val view: View?
    internal val renderable: Renderable?
    internal val modifier: ModifierProtocol

    internal constructor(content: View, modifier: ModifierProtocol = RenderModifier()) {
        // Don't copy
        this.view = content
        this.renderable = content as? Renderable
        this.modifier = modifier.sref()
    }

    internal constructor(content: Renderable, modifier: ModifierProtocol = RenderModifier()) {
        // Don't copy
        this.renderable = content
        this.view = content as? View
        this.modifier = modifier.sref()
    }
    ...
    override fun Evaluate(context: ComposeContext, options: Int): kotlin.collections.List<Renderable> {
        ...
        val matchtarget_1 = modifier.Evaluate(content = view, context = context, options = options)
        if (matchtarget_1 != null) {
            val renderables = matchtarget_1
    ...
    override fun strip(): Renderable {
        return (renderable?.strip() ?: EmptyView()).sref()
    }
    ...
    override fun <R> forEachModifier(perform: (ModifierProtocol) -> R?): R? {
        ...
        val matchtarget_2 = action(modifier)
        if (matchtarget_2 != null) {
            val ret = matchtarget_2
            return ret.sref()
        } else {
    ...
    override fun shouldProduceLazyItems(): Boolean {
    ...
    companion object {
        ...
        internal fun apply(modifiers: kotlin.collections.List<ModifierProtocol>, to: View): View {
            val view = to
            if (modifiers.size <= 0) {
                return view.sref()
            }
            val modifiedView = apply(modifiers = modifiers.drop(1), to = view)
            return ModifiedContent(content = modifiedView, modifier = modifiers[0])
        }

        /// Apply the given modifiers to a `Renderable`.
        internal fun apply(modifiers: kotlin.collections.List<ModifierProtocol>, to: Renderable): Renderable {
            val renderable = to
            if (modifiers.size <= 0) {
                return renderable.sref()
            }
            val modifiedRenderable = apply(modifiers = modifiers.drop(1), to = renderable)
            return ModifiedContent(content = modifiedRenderable, modifier = modifiers[0])
        }

        /// Apply the given modifiers to a render.
        @Composable
        internal fun RenderWithModifiers(modifiers: kotlin.collections.List<ModifierProtocol>, context: ComposeContext, content: @Composable (ComposeContext) -> Unit) {
            if (modifiers.size <= 0) {
                content(context)
                return
            }
        ...
        @Composable
        internal fun Evaluate(content: View, context: ComposeContext, options: Int): kotlin.collections.List<Renderable> {
