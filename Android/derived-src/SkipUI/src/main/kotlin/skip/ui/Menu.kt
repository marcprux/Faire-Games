    ...
    companion object {
        ...
        internal fun RenderDropdownMenuItems(for_: kotlin.collections.List<Renderable>, selection: Hashable? = null, context: ComposeContext, replaceMenu: (Menu?) -> Unit) {
            ...
            for (renderable in renderables.sref()) {
                ...
                val itemModifier = accessibilityModifier(for_ = renderable, context = context)
                ...
                if (matchtarget_2 != null) {
                    ...
                    RenderDropdownMenuItem(for_ = button.label, context = context, modifier = itemModifier, tintColor = tintColor, isSelected = isSelected) { ->
                        button.action()
                        replaceMenu(null)
                    }
                } else {
                        ...
                        if (matchtarget_5 != null) {
                            ...
                            if (matchtarget_6 != null) {
                                ...
                                (menu.label.Evaluate(context = context, options = 0).firstOrNull()?.strip() as? Button)?.let { button ->
                                    RenderDropdownMenuItem(for_ = button.label, context = context, modifier = itemModifier) { -> replaceMenu(menu) }
                                }
                            } else {
        ...
        @Composable
        private fun accessibilityModifier(for_: Renderable, context: ComposeContext): Modifier {
            val renderable = for_
            var collected: Array<RenderModifier> = arrayOf()
            renderable.forEachModifier(fun(mod: ModifierProtocol): Boolean? {
                (mod as? RenderModifier)?.let { renderMod ->
                    if (renderMod.role == ModifierRole.accessibility) {
            ...
            for (renderMod in collected.reversed()) {
                renderMod.modifierAction?.let { modAction ->
                    var ctx = context.sref()
        ...
        @Composable
        private fun RenderDropdownMenuItem(for_: ComposeBuilder, context: ComposeContext, modifier: Modifier = Modifier, tintColor: Color? = null, isSelected: Boolean? = null, action: () -> Unit) {
            val view = for_
