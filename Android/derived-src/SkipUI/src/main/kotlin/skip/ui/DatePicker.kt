...
class DatePicker: View, Renderable, skip.lib.SwiftProjecting {
    ...
    private fun RenderPickerContent(context: ComposeContext) {
        ...
        val timeZone = EnvironmentValues.shared.timeZone.sref()
        val (hour, minute) = hourAndMinute(from = date, timeZone = timeZone)
        ...
        RenderDatePicker(context = context, isPresented = isDatePickerPresented, tintColor = tintColor, timeZone = timeZone) { it -> didSelect(date = it, hour = hour, minute = minute, timeZone = timeZone) }
        RenderTimePicker(context = context, isPresented = isTimePickerPresented, tintColor = tintColor, hour = hour, minute = minute) { it, it_1 -> didSelect(date = date, hour = it, minute = it_1, timeZone = timeZone) }
    }

    @Composable
    private fun RenderDatePicker(context: ComposeContext, isPresented: MutableState<Boolean>, tintColor: androidx.compose.ui.graphics.Color, timeZone: TimeZone, dateSelected: (Date) -> Unit) {
        if (!isPresented.value) {
            return
        }
        val initialMillis = datePickerMillis(from = selection.wrappedValue, timeZone = timeZone)
        ...
        val minMillis: Long? = if (minDate != null) datePickerMillis(from = minDate!!, timeZone = timeZone) else null
        val maxMillis: Long? = if (maxDate != null) datePickerMillis(from = maxDate!!, timeZone = timeZone) else null
    ...
    private fun datePickerMillis(from: Date, timeZone: TimeZone): Long {
        val date = from
        val timeZoneOffset = Double(timeZone.secondsFromGMT(for_ = date))
        return Long((date.timeIntervalSince1970 + timeZoneOffset) * 1000.0)
    }

    private fun date(fromDatePickerMillis: Long, timeZone: TimeZone): Date {
        val millis = fromDatePickerMillis
        val utcDate = Date(timeIntervalSince1970 = Double(millis) / 1000.0)
        val timeZoneOffset = Double(timeZone.secondsFromGMT(for_ = utcDate))
    ...
    private fun didSelect(date: Date, hour: Int, minute: Int, timeZone: TimeZone = TimeZone.current) {
        // Subtract out any existing hour and minute from the given date, then add the selected values
        val (baseHour, baseMinute) = hourAndMinute(from = date, timeZone = timeZone)
    ...
    private fun hourAndMinute(from: Date, timeZone: TimeZone = TimeZone.current): Tuple2<Int, Int> {
        val date = from
        var calendar = Calendar.current.sref()
