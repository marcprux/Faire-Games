...
private class ColorSet: Decodable {
    ...
    internal class ColorComponents: Decodable {
        ...
        internal val color: Color
            get() {
                val redValue = Companion.parseComponent(red, defaultValue = 0.0)
                val greenValue = Companion.parseComponent(green, defaultValue = 0.0)
                val blueValue = Companion.parseComponent(blue, defaultValue = 0.0)
                val alphaValue = Companion.parseComponent(alpha, defaultValue = 1.0)
        ...
        companion object: DecodableCompanion<ColorSet.ColorComponents> {
            ...
            private fun hexDigitValue(character: Char): Int? {
                when (character) {
                    '0' -> return 0
                    '1' -> return 1
                    '2' -> return 2
            ...
            private fun parseHexComponent(value: String): Double? {
                val hexValue = value.dropFirst(2)
                ...
                for (character in hexValue) {
                    val digit_0 = Companion.hexDigitValue(character)
                    if (digit_0 == null) {
                        return null
                    }

                    intValue = intValue * 16 + digit_0
                }

                return Double(intValue) / 255.0
            }

            private fun parseComponent(value: String?, defaultValue: Double): Double {
                val value_0 = value?.trimmingCharacters(in_ = CharacterSet.whitespacesAndNewlines)
                if ((value_0 == null) || value_0.isEmpty) {
                    return defaultValue
                }

                if (value_0.hasPrefix("0x") || value_0.hasPrefix("0X")) {
                    return Companion.parseHexComponent(value_0) ?: defaultValue
                }

                // "8-bit (0-255)" input method (e.g. "148"): a bare integer with no decimal point, normalized by /255.
                if (!value_0.contains(".")) {
                    Int(value_0)?.let { intValue ->
                        return Double(intValue) / 255.0
                    }
                }

                return Double(value_0) ?: defaultValue
            }
