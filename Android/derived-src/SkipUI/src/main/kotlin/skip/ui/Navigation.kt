...
class NavigationStack: View, Renderable, skip.lib.SwiftProjecting {
    ...
    override fun Render(context: ComposeContext) {
        ...
        val destinationLayoutHints = rememberSaveable(stateSaver = context.stateSaver as Saver<Preference<Dictionary<AnyHashable, NavigationStackLayoutHints>>, Any>) { -> mutableStateOf(Preference<Dictionary<AnyHashable, NavigationStackLayoutHints>>(key = NavigationDestinationLayoutHintsPreferenceKey::class)) }
        val destinationLayoutHintsCollector = PreferenceCollector<Dictionary<AnyHashable, NavigationStackLayoutHints>>(key = NavigationDestinationLayoutHintsPreferenceKey::class, state = destinationLayoutHints, isErasable = false)
        ...
        CompositionLocalProvider(providedNavigator) { ->
            ...
            IgnoresSafeAreaLayout(expandInto = ignoresSafeAreaEdges, checkEdges = ignoresSafeAreaEdges, logTag = "NavigationStack") { _, _ ->
                ComposeContainer(modifier = context.modifier, fillWidth = true, fillHeight = true) { modifier ->
                    ...
                    val defaults = NavDisplayTransitionOptions.navigationStackDefaults
                    val transitions = EnvironmentValues.shared._navigationStackTransitions?.invoke(defaults) ?: defaults
                    NavDisplay(backStack = navBackStack, modifier = modifier, onBack = { -> navigator.value.navigateBack() }, transitionSpec = { -> transitions.transitionSpec ?: defaultTransitionSpec<NavKey>()() }, popTransitionSpec = { -> transitions.popTransitionSpec ?: defaultPopTransitionSpec<NavKey>()() }, predictivePopTransitionSpec = { edge -> transitions.predictivePopTransitionSpec ?: defaultPredictivePopTransitionSpec<NavKey>()(edge) }, entryDecorators = decoratorList, entryProvider = entryProvider)
                }
            }
        }
    }
    ...
    private fun RenderEntry(navigator: MutableState<Navigator>, toolbarContent: MutableState<Preference<ToolbarContentPreferences>>, arguments: NavigationEntryArguments, context: ComposeContext, content: @Composable (ComposeContext) -> Unit) {
        ...
        val presentationTopPx = arguments.safeArea?.presentationBoundsPx?.top ?: 0.0f
        val isInitialRootPreferencePass = remember { -> mutableStateOf(arguments.isRoot) }
        val reservesInitialRootTopBar = isInitialRootPreferencePass.value && arguments.isRoot && !hasTitle && (topBarPreferences?.visibility ?: Visibility.automatic) == Visibility.automatic
        val effectiveTitleDisplayMode = navigator.value.titleDisplayMode(for_ = state, hasTitle = hasTitle || reservesInitialRootTopBar, preference = titleDisplayPreference)
        ...
        val showsTopBar = showTopBar || reservesInitialRootTopBar
        if (showsTopBar) {
            modifier = modifier.nestedScroll(scrollBehavior.nestedScrollConnection)
        }
        modifier = modifier.then(context.modifier)

        val shouldReserveInitialTopBarSpace = showTopBar || reservesInitialRootTopBar
        val estimatedCollapsedTopBarHeight = 64.dp.sref()
        val estimatedExpandedTopBarHeight = 112.dp.sref()
        val estimatedTopBarHeight = (if (isInlineTitleDisplayMode) estimatedCollapsedTopBarHeight else estimatedExpandedTopBarHeight).sref()
        val safeAreaTopPx = arguments.safeArea?.safeBoundsPx?.top ?: 0.0f
        val hasAbsoluteTopSystemBar = arguments.safeArea?.absoluteSystemBarEdges?.contains(Edge.Set.top) == true
        val estimatedTopBarBottomPx = if (shouldReserveInitialTopBarSpace) with(density) { -> safeAreaTopPx + estimatedTopBarHeight.toPx() } else 0.0f
        val estimatedTopBarBottomInPresentationPx = max(0.0f, estimatedTopBarBottomPx - presentationTopPx)
        val topBarBottomPx = remember { -> mutableStateOf(estimatedTopBarBottomInPresentationPx) }
        val measuredTopBarIsInline = remember { -> mutableStateOf(isInlineTitleDisplayMode) }
        val measuredTopBarSafeAreaTopPx = remember { -> mutableStateOf<Float?>(null) }
        val measuredTopBarPresentationTopPx = remember { -> mutableStateOf<Float?>(null) }
        LaunchedEffect(showsTopBar, isInitialRootPreferencePass.value) { ->
            if (!showsTopBar && !isInitialRootPreferencePass.value) {
                ...
                measuredTopBarSafeAreaTopPx.value = null
                measuredTopBarPresentationTopPx.value = null
            }
        }
        var safeAreaMatchesMeasuredTopBar = false
        measuredTopBarSafeAreaTopPx.value.sref()?.let { measuredSafeAreaTopPx ->
            measuredTopBarPresentationTopPx.value.sref()?.let { measuredPresentationTopPx ->
                safeAreaMatchesMeasuredTopBar = measuredSafeAreaTopPx == safeAreaTopPx && measuredPresentationTopPx == presentationTopPx
            }
        }
        val isPresentedAsSheet = presentationTopPx > 0.0f && arguments.safeArea?.absoluteSystemBarEdges?.contains(Edge.Set.top) != true
        val hasCurrentTopBarMeasurement = measuredTopBarIsInline.value == isInlineTitleDisplayMode && (isPresentedAsSheet || safeAreaMatchesMeasuredTopBar) && topBarBottomPx.value > 0.0f
        val effectiveTopBarBottomPx = if (shouldReserveInitialTopBarSpace) presentationTopPx + (if (hasCurrentTopBarMeasurement) topBarBottomPx.value else estimatedTopBarBottomInPresentationPx) else 0.0f
        ...
        val topBar: @Composable () -> Unit = { ->
            ...
            AnimatedVisibility(visible = showsTopBar, modifier = Modifier.fillMaxWidth(), enter = topBarEnter, exit = topBarExit, label = "NavigationTopBar") { ->
                ...
                MaterialTheme(colorScheme = materialColorScheme) { ->
                    ...
                    val isSystemBackground = topBarPreferences?.isSystemBackground == true || reservesInitialRootTopBar
                    ...
                    EnvironmentValues.shared.setValues(l@{ it ->
                        ...
                        var topBarModifier = Modifier.zIndex(1.1f)
                            ...
                            .onGloballyPositioned { coordinates ->
                                val heightPx = Float(coordinates.size.height)
                                if (heightPx > 0.0f) {
                        ...
                        val topBarWindowInsets = (if (hasAbsoluteTopSystemBar) TopAppBarDefaults.windowInsets else WindowInsets(0.dp, 0.dp, 0.dp, 0.dp)).sref()
            ...
            MaterialTheme(colorScheme = materialColorScheme) { ->
                ...
                EnvironmentValues.shared.setValues(l@{ it ->
                    ...
                    PaddingLayout(padding = EdgeInsets(top = 0.0, leading = 0.0, bottom = Double(-bottomPadding.value), trailing = 0.0), context = context.content()) { context ->
                        ...
                        val usesBottomSystemBarInset = EnvironmentValues.shared._isEdgeToEdge == true && arguments.safeArea?.absoluteSystemBarEdges?.contains(Edge.Set.bottom) == true
                        val windowInsets = (if (usesBottomSystemBarInset) BottomAppBarDefaults.windowInsets else WindowInsets(bottom = 0.dp)).sref()
        ...
        if (layoutImplementationVersion < 2) {
            ...
            Column(modifier = modifier.background(Color.background.colorImpl())) { ->
                ...
                val contentSafeArea = arguments.safeArea?.insetting(Edge.top, to = effectiveTopBarBottomPx)?.insetting(Edge.bottom, to = bottomBarTopPx.value)
                // Inset manually for any edge where our container ignored the safe area, but we aren't showing a bar
                val topPadding = (if (effectiveTopBarBottomPx <= 0.0f && arguments.ignoresSafeAreaEdges.contains(Edge.Set.top)) WindowInsets.safeDrawing.asPaddingValues().calculateTopPadding() else 0.dp).sref()
            ...
            Box(modifier = modifier.background(Color.background.colorImpl()).fillMaxSize()) { ->
                ...
                arguments.safeArea?.let { safeArea ->
                    val clampedTopBarBottomPxValue: Float = max(effectiveTopBarBottomPx, safeArea.safeBoundsPx.top)
                ...
                val topBarBottomDp: Dp
                if (showsTopBar) {
                    if (isInlineTitleDisplayMode) {
                        val localTopBarBottomPx = max(0.0f, effectiveTopBarBottomPx - (arguments.safeArea?.presentationBoundsPx?.top ?: 0.0f))
                        topBarBottomDp = with(density) { -> localTopBarBottomPx.toDp() }
                    } else {
                        val localSafeAreaTopDp = with(density) { -> max(0.0f, safeAreaTopPx - presentationTopPx).toDp() }
                        val topBarHeightDp = estimatedCollapsedTopBarHeight + ((estimatedExpandedTopBarHeight - estimatedCollapsedTopBarHeight) * Float(1.0 - scrollBehavior.state.collapsedFraction))
                        topBarBottomDp = localSafeAreaTopDp + topBarHeightDp
                    }
                } else {
                    topBarBottomDp = 0.dp.sref()
                }

                val topPadding = (if (arguments.ignoresSafeAreaEdges.contains(Edge.Set.top)) max(topBarBottomDp, safeTopDp) else topBarBottomDp).sref()
                val bottomPadding: Dp

                val topBarUnderlayColor: androidx.compose.ui.graphics.Color?
                if (topBarPreferences?.backgroundVisibility == Visibility.hidden) {
                    topBarUnderlayColor = null
                } else if (topBarPreferences?.backgroundVisibility == Visibility.visible) {
                    topBarUnderlayColor = topBarPreferences?.background?.asColor(opacity = 1.0, animationContext = null) ?: Color.systemBarBackground.colorImpl()
                ...
                if ((showsTopBar && topPadding.value > 0.0f) && (topBarUnderlayColor != null)) {
                ...
                if (bottomBarHeightPx.value > 0.0f) {
                    bottomPadding = with(density) { -> bottomBarHeightPx.value.toDp() }
                } else if (arguments.ignoresSafeAreaEdges.contains(Edge.Set.bottom)) {
                    bottomPadding = max(0.dp, WindowInsets.safeDrawing.asPaddingValues().calculateBottomPadding() - WindowInsets.ime.asPaddingValues().calculateBottomPadding())
                } else {
                    bottomPadding = 0.dp.sref()
                }

                val bottomBarUnderlayColor: androidx.compose.ui.graphics.Color?
                if (bottomBarPreferences?.backgroundVisibility == Visibility.hidden) {
                    bottomBarUnderlayColor = null
                } else if (bottomBarPreferences?.backgroundVisibility == Visibility.visible) {
                    bottomBarUnderlayColor = bottomBarPreferences?.background?.asColor(opacity = 1.0, animationContext = null) ?: Color.systemBarBackground.colorImpl()
                ...
                if ((bottomPadding.value > 0.0f) && (bottomBarUnderlayColor != null)) {
                    Box(modifier = Modifier
...
@androidx.annotation.Keep
class NavDisplayTransitionOptions: skip.lib.SwiftProjecting {
    @androidx.annotation.Keep
    enum class TransitionPreset: skip.lib.SwiftProjecting {
        default,
        slideIn,
        slideOut,
        slideInAndFade,
        ...
        internal val contentTransform: ContentTransform?
            get() {
                when (this) {
                    NavDisplayTransitionOptions.TransitionPreset.default -> {
                        // This will use NavDisplay.defaultTransitionSpec, .defaultPopTransitionSpec, and .defaultPredictivePopTransitionSpec
                        return null
                    }
                    NavDisplayTransitionOptions.TransitionPreset.slideIn -> {
                        return slideInHorizontally { it -> it }.togetherWith(slideOutHorizontally { it -> -it })
                    }
                    NavDisplayTransitionOptions.TransitionPreset.slideOut -> {
                        return slideInHorizontally { it -> -it }.togetherWith(slideOutHorizontally { it -> it })
                    }
                    NavDisplayTransitionOptions.TransitionPreset.slideInAndFade -> {
                        return (slideInHorizontally { it -> it } + fadeIn()).togetherWith(slideOutHorizontally { it -> -it } + fadeOut())
                    }
                    NavDisplayTransitionOptions.TransitionPreset.slideOutAndFade -> {
                        return (slideInHorizontally { it -> -it } + fadeIn()).togetherWith(slideOutHorizontally { it -> it } + fadeOut())
                    }
                    NavDisplayTransitionOptions.TransitionPreset.fade -> return fadeIn().togetherWith(fadeOut())
                    NavDisplayTransitionOptions.TransitionPreset.none -> return fadeIn(animationSpec = tween(0)).togetherWith(fadeOut(animationSpec = tween(0)))
                }
            }
    ...
    constructor(transitionSpec: ContentTransform?, popTransitionSpec: ContentTransform?, predictivePopTransitionSpec: ContentTransform?) {
        this.transitionSpec = transitionSpec.sref()
        this.popTransitionSpec = popTransitionSpec.sref()
        this.predictivePopTransitionSpec = predictivePopTransitionSpec.sref()
    }

    constructor(transitionSpecs: ContentTransform?) {
        this.transitionSpec = transitionSpecs.sref()
        this.popTransitionSpec = transitionSpecs.sref()
        this.predictivePopTransitionSpec = transitionSpecs.sref()
    }

    constructor(transitionPreset: NavDisplayTransitionOptions.TransitionPreset) {
        this.transitionSpec = transitionPreset.contentTransform.sref()
        this.popTransitionSpec = transitionPreset.contentTransform.sref()
        this.predictivePopTransitionSpec = transitionPreset.contentTransform.sref()
    }

    constructor(transition: NavDisplayTransitionOptions.TransitionPreset = NavDisplayTransitionOptions.TransitionPreset.default, popTransition: NavDisplayTransitionOptions.TransitionPreset = NavDisplayTransitionOptions.TransitionPreset.default, predictivePopTransition: NavDisplayTransitionOptions.TransitionPreset = NavDisplayTransitionOptions.TransitionPreset.default) {
        this.transitionSpec = transition.contentTransform.sref()
        this.popTransitionSpec = popTransition.contentTransform.sref()
        this.predictivePopTransitionSpec = predictivePopTransition.contentTransform.sref()
    }

    fun copy(transitionSpec: ContentTransform? = this.transitionSpec, popTransitionSpec: ContentTransform? = this.popTransitionSpec, predictivePopTransitionSpec: ContentTransform? = this.predictivePopTransitionSpec): NavDisplayTransitionOptions = NavDisplayTransitionOptions(transitionSpec = transitionSpec, popTransitionSpec = popTransitionSpec, predictivePopTransitionSpec = predictivePopTransitionSpec)

    /// Preset-based updates. Pass `nil` for a slot to leave `self`’s value unchanged.
    fun copy(transition: NavDisplayTransitionOptions.TransitionPreset? = null, popTransition: NavDisplayTransitionOptions.TransitionPreset? = null, predictivePopTransition: NavDisplayTransitionOptions.TransitionPreset? = null): NavDisplayTransitionOptions {
        return copy(transitionSpec = if (transition == NavDisplayTransitionOptions.TransitionPreset.default) null else (transition?.contentTransform ?: transitionSpec), popTransitionSpec = if (popTransition == NavDisplayTransitionOptions.TransitionPreset.default) null else (popTransition?.contentTransform ?: popTransitionSpec), predictivePopTransitionSpec = if (predictivePopTransition == NavDisplayTransitionOptions.TransitionPreset.default) null else (predictivePopTransition?.contentTransform ?: predictivePopTransitionSpec))
    }
    ...
    companion object {
        ...
        val navigationStackDefaults: NavDisplayTransitionOptions
            get() = NavDisplayTransitionOptions(transition = NavDisplayTransitionOptions.TransitionPreset.slideInAndFade, popTransition = NavDisplayTransitionOptions.TransitionPreset.slideOutAndFade, predictivePopTransition = NavDisplayTransitionOptions.TransitionPreset.slideOut)

        val tabViewDefaults: NavDisplayTransitionOptions
            get() = NavDisplayTransitionOptions()
    }
}
