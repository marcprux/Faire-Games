...
@Composable
internal fun IgnoresSafeAreaLayout(expandInto: Edge.Set, checkEdges: Edge.Set = Edge.Set.of(), modifier: Modifier = Modifier, bottomAdjacencyTolerancePx: Float = 0.1f, logTag: String = "", target: @Composable (IntRect, Edge.Set) -> Unit) {
    ...
    val effectiveBottomAdjacencyTolerancePx = max(bottomAdjacencyTolerancePx, safeArea_0.presentationBoundsPx.bottom - safeArea_0.safeBoundsPx.bottom)
    ...
    EnvironmentValues.shared.setValues(l@{ it ->
        ...
        Layout(modifier = modifier.onGloballyPositionedInWindow { it ->
            ...
            val newEdges = adjacentSafeAreaEdges(bounds = it, safeArea = safeArea_0, isRTL = isRTL, checkEdges = probeEdges, bottomTolerancePx = effectiveBottomAdjacencyTolerancePx)
            if (!logTag.isEmpty) {
            ...
            // the min constraints
            val updatedConstraints = constraints.copy(maxWidth = constraint(constraints.maxWidth, adding = expansionLeft + expansionRight, atLeast = constraints.minWidth), maxHeight = constraint(constraints.maxHeight, adding = expansionTop + expansionBottom, atLeast = constraints.minHeight))
...
private fun adjacentSafeAreaEdges(bounds: Rect, safeArea: SafeArea, isRTL: Boolean, checkEdges: Edge.Set, bottomTolerancePx: Float = 0.1f): Edge.Set {
...
private fun constraint(value: Int, adding: Int, atLeast: Int): Int {
    if (value == Int.MAX_VALUE) {
        return value
    }
