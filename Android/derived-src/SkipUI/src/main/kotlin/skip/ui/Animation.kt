...
class Animation: skip.lib.SwiftProjecting {
    ...
    private class BridgeFrame {
        internal val animation: Animation?
        internal val token: StateMutationTransactionToken?

        internal constructor(animation: Animation?, token: StateMutationTransactionToken?) {
            this.animation = animation.sref()
            this.token = token
        }
    }
    ...
    @androidx.annotation.Keep
    companion object {
        fun preBodyWithAnimation(animation: Animation?): Boolean {
            ...
            val wasInside = StateTracking.currentTransaction != null
            var stack = bridgeFrameStack.get()
            if (stack == null) {
                stack = mutableListOf<Animation.BridgeFrame>()
                bridgeFrameStack.set(stack)
            }
            val token: StateMutationTransactionToken?
            if (animation != null) {
        ...
        fun postBodyWithAnimation() {
            val stack_0 = bridgeFrameStack.get()
            if ((stack_0 == null) || stack_0.isEmpty()) {
                return
            }
            val frame = stack_0.removeAt(stack_0.size - 1)
            frame.token.sref()?.let { token ->
                StateTracking.popTransaction(token)
            }
            frame.animation.sref()?.let { animation ->
        ...
        private val bridgeFrameStack: ThreadLocal<MutableList<Animation.BridgeFrame>> = ThreadLocal()
        ...
        private var recentWithAnimationAnimation: Animation? = null
            get() = field.sref({ this.recentWithAnimationAnimation = it })
            set(newValue) {
                field = newValue.sref()
            }
        private var recentWithAnimationGeneration: Long = 0
            get() = field.sref({ this.recentWithAnimationGeneration = it })
            set(newValue) {
                field = newValue.sref()
            }
        ...
        /// are unaffected.
        private var bridgedComposition = false
        ...
        private var bridgedProvenance = false
        ...
        fun primeBridgedProvenance(animation: Animation?) {
        ...
        internal fun markRecentWithAnimation(animation: Animation?) {
            recentWithAnimationAnimation = animation
            recentWithAnimationGeneration += 1
            val generation = recentWithAnimationGeneration.sref()
            GlobalScope.async(Dispatchers.Main) { ->
                withFrameNanos { _ -> }
                if (recentWithAnimationGeneration == generation) {
                    recentWithAnimationAnimation = null
                }
            }
        }
        ...
        internal fun resetRecentWithAnimationForTesting() {
            recentWithAnimationAnimation = null
            recentWithAnimationGeneration += 1
            bridgedComposition = false
            bridgedProvenance = false
            bridgeFrameStack.set(null)
        ...
        internal fun current(isAnimating: Boolean): Animation? {
            var ambient = EnvironmentValues.shared._animation.sref()
            if (ambient == null) {
                ambient = recentWithAnimationAnimation.sref()
            }
        ...
        @Composable
        internal fun current(isAnimating: Boolean, animTx: StateMutationTransaction?): Animation? {
            var ambient = EnvironmentValues.shared._animation.sref()
            if (ambient == null) {
                (animTx as? Transaction)?.let { tx ->
                    if (!tx.disablesAnimations) {
                        ambient = tx.animation.sref()
                    }
                }
            }
            if ((ambient == null) && (animTx == null) && bridgedComposition && !bridgedProvenance) {
        ...
        @Composable
        private fun resolve(isAnimating: Boolean, ambient: Animation?): Animation? {
        ...
        internal val isInWithAnimation: Boolean
            get() = recentWithAnimationAnimation != null
...
enum class AnimationCompletionCriteria {
    logicallyComplete,
    removed;
...
@Composable
internal fun <T, VectorT> toAnimatable(value: T, converter: TwoWayConverter<T, VectorT>, context: ComposeContext, animTx: StateMutationTransaction?): Animatable<T, VectorT> where T: Any, VectorT: AnimationVector {
    val resetValue = rememberSaveable(stateSaver = context.stateSaver as Saver<T?, Any>) { -> mutableStateOf<T?>(null) }
    val animatable = remember { -> Animatable(resetValue.value ?: value, converter) }
    val isAnimating = animatable.isRunning || animatable.value != animatable.targetValue
    if (isAnimating || animatable.value != value) {
        val animation = Animation.current(isAnimating = isAnimating, animTx = animTx)
        LaunchedEffect(value, animation) { ->
            if (animation != null) {
                if (animation.isInfinite) {
                    resetValue.value = animatable.value // Remember infinite animation start value
...
@Composable
internal fun Float.asAnimatable(context: ComposeContext, animTx: StateMutationTransaction?): Animatable<Float, AnimationVector1D> {
    return toAnimatable(value = this, converter = TwoWayConverter({ it -> AnimationVector1D(it) }, { it -> it.value }), context = context, animTx = animTx)
}
...
/// Return an animatable version of this value (instrumented site: per-slot provenance only).
@Composable
internal fun Tuple2<Float, Float>.asAnimatable(context: ComposeContext, animTx: StateMutationTransaction?): Animatable<Tuple2<Float, Float>, AnimationVector2D> {
    return toAnimatable(value = this, converter = TwoWayConverter({ it -> AnimationVector2D(it.element0, it.element1) }, { it -> Tuple2(it.v1, it.v2) }), context = context, animTx = animTx)
}
