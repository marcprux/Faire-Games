...
class DismissAction: skip.lib.SwiftProjecting {
    val action: () -> Unit
...
@androidx.annotation.Keep
class OpenURLAction: skip.lib.SwiftProjecting {
    @androidx.annotation.Keep
    class Result: skip.lib.SwiftProjecting {
        val rawValue: Int
        val url: URL?

        constructor(rawValue: Int, url: URL? = null) {
        ...
        companion object {
            ...
            val handled = Result(rawValue = 0) // For bridging
            val discarded = Result(rawValue = 1) // For bridging
            val systemAction = Result(rawValue = 2) // For bridging
            fun systemAction(url: URL): OpenURLAction.Result = Result(rawValue = 2, url = url) // For bridging
            fun systemAction(url: URL, prefersInApp: Boolean): OpenURLAction.Result = Result(rawValue = 2, url = url) // For bridging
        }
    }

    val handler: (URL) -> OpenURLAction.Result
    val systemHandler: ((URL) -> Unit)?
...
@androidx.annotation.Keep
class RefreshAction: skip.lib.SwiftProjecting {
    ...
    constructor(bridgedAction: (CompletionHandler) -> Unit, unusedp: Any? = null) {
        this.action = { -> Async.run {
    ...
    suspend operator fun invoke(): Unit = Async.run {
        action()
    }

    fun run(completion: () -> Unit) {
        Task { ->
            action()
            completion()
        }
