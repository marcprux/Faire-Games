...
fun View.withDocumentPreview(isPresented: Binding<Boolean>, documentURL: URL?, filename: String? = null, type: String? = null): View {
    return ComposeBuilder l@{ composectx: ComposeContext ->
        ...
        return@l onChange(of = isPresented.wrappedValue) { presented ->
            if (presented == true) {
                ...
                val file = java.io.File(documentURL!!.absoluteString)
                val uri = androidx.core.content.FileProvider.getUriForFile(context.asActivity(), context.getPackageName() + ".fileprovider", file)
                ...
                if (mimeType.isEmpty) {
                    ...
                    val contentResolver: android.content.ContentResolver = context.contentResolver.sref()
