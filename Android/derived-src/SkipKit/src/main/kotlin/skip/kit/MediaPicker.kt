...
fun View.withMediaPicker(type: MediaPickerType, isPresented: Binding<Boolean>, allowsMultipleSelection: Boolean, selectedImageURLs: Binding<Array<URL>>): View {
    return ComposeBuilder l@{ composectx: ComposeContext ->
        when (type) {
            MediaPickerType.library -> {
                if (allowsMultipleSelection) {
                    val pickImagesLauncher = rememberLauncherForActivityResult(contract = ActivityResultContracts.GetMultipleContents()) { uris ->
                        isPresented.wrappedValue = false
                        var urls = Array<URL>()
                    ...
                    val pickImageLauncher = rememberLauncherForActivityResult(contract = ActivityResultContracts.GetContent()) { uri ->
                        ...
                        logger.log("pickImageLauncher: ${uri}")
                        if (uri != null) {
                            selectedImageURLs.wrappedValue = arrayOf(URL(platformValue = java.net.URI.create(uri.toString())))
                        }
                    }
            ...
            MediaPickerType.camera -> {
                withMediaPicker(type = type, isPresented = isPresented, selectedImageURL = Binding(get = { -> selectedImageURLs.wrappedValue.first }, set = { it ->
                    if (it != null) {
                        val value = it.sref()
                        selectedImageURLs.wrappedValue = arrayOf(value)
                    } else {
                        selectedImageURLs.wrappedValue = arrayOf()
                    }
                })).Compose(composectx)
            }
        }
